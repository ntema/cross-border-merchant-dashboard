...
export default function LedgerTable({ transactions, isLoading, error, tenant }: LedgerTableProps) {
  // Task 3: Fintech UI Data Handling
  // Converts minor unit amounts (e.g. 125050 cents) to proper currency format
  const formatAmount = (amountInMinor: number, currency: string = "USD") => {
    // Divide by 100 to convert from minor units (no floating point issues)
    const amount = amountInMinor / 100;

    return new Intl.NumberFormat(tenant?.locale || "en-US", {
      style: "currency",
      currency: currency,
      // Ensures consistent decimal places for financial data
      minimumFractionDigits: 2,
      maximumFractionDigits: 2,
    }).format(amount);
  };

  // Task 3: Format UTC timestamp to local browser time
  const formatDate = (utcDateString: string) => {
    const date = new Date(utcDateString);
    return new Intl.DateTimeFormat(tenant?.locale || 'en-US', {
      year: 'numeric',
      month: 'short',
      day: 'numeric',
      hour: 'numeric',
      minute: '2-digit',
      timeZoneName: 'short', // Shows local timezone (e.g. GMT+1)
    }).format(date);
  };
          ...
          <tbody className="divide-y divide-border">
            {transactions.map((tx) => (
              <tr key={tx.id} className="hover:bg-muted/50 transition-colors">
                ...
                <td className="p-6">
                  {/* Updated: Uses Intl.DateTimeFormat for proper local timezone display */}
                ...
                <td className="p-6 text-right font-semibold">
                  {/* Updated: Safe minor unit to currency conversion */}
